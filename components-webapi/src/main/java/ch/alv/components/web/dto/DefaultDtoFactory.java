...
public class DefaultDtoFactory implements DtoFactory {
    ...
    public Dto createDtoFromRequestBody(String requestBody, Endpoint endpoint) {
        if (StringHelper.isEmpty(requestBody)) {
            throw new DtoFactoryException("requestBody param must not be empty.");
        }
        ...
        if (matcher.find()) {
            ...
            if (StringHelper.isEmpty(value)) {
                throw new DtoFactoryException("Illegal string source.");
            }
        }
        if (endpoint == null) {
    ...
    @Override
    public Dto createDtoFromEntity(Identifiable entity, Endpoint endpoint) {
        if (entity == null) {
            return null;
        }
    ...
    private String createURL(Identifiable entity, Endpoint endpoint) {
