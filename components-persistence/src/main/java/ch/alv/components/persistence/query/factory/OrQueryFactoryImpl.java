...
/**
 * Combines all parameters to an OR-related query.
 *
 * @author seco-hrf
 * @since 1.0.0
 */
public class OrQueryFactoryImpl implements QueryFactory {

    @Override
    public String getName() {
        return "orQueryFactoryImpl";
    }
    ...
    public SelectQuery createQuery(Search config, Class<?> entityClass, ParamValuesProvider valuesProvider) {
        ...
        // Know if there was already a valid param added. If true, .where() must be called, else .or().
